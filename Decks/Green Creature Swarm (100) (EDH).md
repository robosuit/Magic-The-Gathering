# Green Creature Swarm (100) (EDH)

Tags: #deck/commander #format/edh #color/green

## Commander
- `G` [[Polukranos, World Eater]] x1 | Legendary Creature - Hydra

---

## Green Creatures (35)
- `G` [[Llanowar Elves]] x1
- `G` [[Fierce Empath]] x1
- `G` [[Imperious Perfect]] x1
- `G` [[Dwynen's Elite]] x1
- `G` [[Wildborn Preserver]] x1
- `G` [[Springbloom Druid]] x1
- `G` [[Priest of Titania]] x1
- `G` [[Reclamation Sage]] x1
- `G` [[Thornweald Archer]] x1
- `G` [[Elvish Regrower]] x1
- `G` [[Mold Adder]] x1
- `G` [[Vizier of the Menagerie]] x1
- `G` [[Rampaging Baloths]] x1
- `G` [[Surrak, the Hunt Caller]] x1
- `G` [[Crowned Ceratok]] x1
- `G` [[Heroes' Bane]] x1
- `G` [[Wildwood Scourge]] x1
- `G` [[Game Warden]] x1
- `G` [[Golden Hind]] x1
- `G` [[Pelakka Wurm]] x1
- `G` [[Elfsworn Giant]] x1
- `G` [[Rhino Predator]] x1
- `G` [[Kavu Predator]] x1
- `G` [[Troll Ascetic]] x1
- `G` [[Pelt Collector]] x1
- `G` [[Manglehorn]] x1
- `G` [[Yavimaya Ape]] x1
- `G` [[Canopy Tactician]] x1
- `G` [[District Guide]] x1
- `G` [[Jade Mage]] x1
- `G` [[Embodiment of Spring]] x1
- `G` [[Elite Scaleguard]] x1
- `G` [[Scute Swarm]] x1
- `G` [[Taunting Elf]] x1
- `G` [[Overgrown Armasaur]] x1

## Green Spells (15)
- `G` [[Kodama's Reach]] x1
- `G` [[Prey Upon]] x1
- `G` [[Overrun]] x1
- `G` [[Primal Might]] x1
- `G` [[Bushwhack]] x1
- `G` [[Circuitous Route]] x1
- `G` [[Regrowth]] x1
- `G` [[Beast Within]] x1
- `G` [[Snakeskin Veil]] x1
- `G` [[Broken Wings]] x1
- `G` [[Ranger's Guile]] x1
- `G` [[Collect the Bounty]] x1
- `G` [[Howl of the Horde]] x1
- `G` [[Commune with Dinosaurs]] x1
- `G` [[Once Upon a Time]] x1

## Green Enchantments (3)
- `G` [[Primeval Bounty]] x1
- `G` [[Garruk's Uprising]] x1
- `G` [[Ordeal of Nylea]] x1

## Colorless (6)
- `C` [[Sol Ring]] x1
- `C` [[Basilisk Collar]] x1
- `C` [[Cultivator's Caravan]] x1
- `C` [[Hedron Archive]] x1
- `C` [[Expedition Map]] x1
- `C` [[Arcane Signet]] x1

## Lands (40)
- `G` [[Forest]] x30
- `C` [[Command Tower]] x1
- `C` [[Evolving Wilds]] x2
- `C` [[Exotic Orchard]] x1
- `C` [[Rogue's Passage]] x1
- `C` [[Cryptic Caves]] x1
- `C` [[Maze of Ith]] x1
- `C` [[Nykthos, Shrine to Nyx]] x1
- `C` [[Scavenger Grounds]] x1
- `C` [[Timber Gorge]] x1

---

**Card Count: 35 + 15 + 3 + 6 + 40 = 99 + 1 Commander = 100 ✓**

